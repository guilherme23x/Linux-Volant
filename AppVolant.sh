...
if [ "$EUID" -eq 0 ]; then
  ...
  echo "Execute apenas: bash install_volant_complete.sh"
  exit 1
fi

echo "=========================================================="
echo "   🎮 INSTALADOR VOLANT CONTROLLER - VERSÃO COMPLETA    "
echo "   📦 Compila automaticamente o APK                      "
echo "=========================================================="
...
if [ -d "$DIR_NAME" ] || [ -d "$HOME/.buildozer" ]; then
    echo "🧹 [1/9] Limpando instalações anteriores..."
    sudo rm -rf "$DIR_NAME"
...
echo "📁 [3/9] Criando estrutura do projeto..."
mkdir "$DIR_NAME"
cd "$DIR_NAME"

# Ambiente Virtual
echo "🐍 [4/9] Configurando Python Virtual Environment..."
python3 -m venv .venv
source .venv/bin/activate

# Instalação de Ferramentas Python
echo "🔧 [5/9] Instalando ferramentas Python..."
pip install --upgrade pip setuptools wheel
pip install Cython==0.29.33
pip install buildozer==1.5.0

# Geração do main.py
echo "📝 [6/9] Gerando main.py otimizado..."
...
<MainLayout>:
    ...
    GamepadButton:
        text: "RB"
        pos_hint: {'right': 0.95, 'center_y': 0.50}
        size_hint: 0.15, 0.10
    ...
    BoxLayout:
        orientation: 'vertical'
        pos_hint: {'right': 0.95, 'center_y': 0.80}
        size_hint: 0.20, 0.4
        Slider:
            id: rt_slider
            orientation: 'vertical'
    ...
    GamepadButton:
        text: "B"
        pos_hint: {'right': 0.85, 'center_y': 0.30}
        size_hint: 0.10, 0.15
    ...
    GamepadButton:
        text: "A"
        pos_hint: {'right': 0.98, 'center_y': 0.20}
        size_hint: 0.10, 0.15
...
class MainLayout(FloatLayout):
    ...
    def send_trigger(self, code, val):
        """
        Envia valores dos gatilhos LT (ABS_Z) e RT (ABS_RZ)
        Range: 0-255 (padrão para gatilhos de controle Xbox)
        """
        threading.Thread(
            target=self.send_payload,
    ...
    def send_tilt(self, val):
        """
        Envia dados do volante (acelerômetro)
        ABS_X: Direção do volante (-255 a 255)
        """
        threading.Thread(
            target=self.send_payload,
    ...
    def update_tilt(self, dt):
        ...
        try:
            ...
            if acc:
                y = acc[1]
                # Mapeia o acelerômetro para o volante (-255 a 255)
                val = (max(min(y, 7.0), -7.0) / 7.0) * 255
...
echo "⚙️  [7/9] Configurando Buildozer..."
buildozer init

# Sobrescreve buildozer.spec
...
echo ""
echo "=========================================================="
echo "🔨 [8/9] INICIANDO COMPILAÇÃO DO APK"
echo "=========================================================="
echo ""
echo "⏳ Isso pode levar de 15 a 45 minutos na primeira vez..."
echo "   (Downloads: SDK, NDK, dependências Python, etc.)"
echo ""
...
if [ -n "$APK_PATH" ]; then
    ...
    echo ""
    echo "🔌 OU instale via USB (com depuração ativada):"
    echo "   adb install $APK_PATH"
else
    echo "❌ ERRO: APK não foi encontrado no diretório bin/"
